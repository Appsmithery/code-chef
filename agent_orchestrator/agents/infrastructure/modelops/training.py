...
class ModelOpsTrainerClient:
    ...
    def wait_for_completion(
        ...
        while (time.time() - start_time) < max_wait:
            ...
            logger.info(f"Job {job_id} status: {status['status']} - waiting {poll_interval}s...")
...
class ModelOpsTrainer:
    ...
    def __init__(self, config_path: Optional[str] = None):
        ...
        self.config_path = config_path or str(
            Path(__file__).parent.parent.parent.parent.parent / "config" / "modelops" / "training_defaults.yaml"
        )
        ...
        logger.info("ModelOpsTrainer initialized")
    ...
    def train_model(
        ...
        if len(df) < 10:
            raise ValueError(f"Insufficient training data: {len(df)} examples (need at least 10)")
    ...
    def monitor_training(
        ...
        if final_status["status"] == "completed":
            ...
            logger.error(f"Training failed: {final_status.get('error', 'Unknown error')}")
