...
async def test_workspace_aware_workflow():
    ...
    expected_fields = ["task_id", "status", "agent", "workspace_context", "linear_project"]
    ...
    if returned_context.get("workspace_name") == TEST_WORKSPACE:
        ...
        print(f"   ❌ workspace_name mismatch: {returned_context.get('workspace_name')}")
        return False
    
    if returned_context.get("github_repo_url") == TEST_GITHUB_REPO:
        print(f"   ✅ github_repo_url: {returned_context['github_repo_url']}")
    else:
        print(f"   ❌ github_repo_url mismatch: {returned_context.get('github_repo_url')}")
        return False
    ...
    if linear_project_2nd.get("id") == cached_project_id:
        ...
        print(f"   ❌ Project ID mismatch: {linear_project_2nd.get('id')} != {cached_project_id}")
        return False
