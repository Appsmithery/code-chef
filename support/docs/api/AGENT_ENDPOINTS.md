...
- `GET /approvals/{approval_id}` — Get details for a specific approval request
  - Returns: `{"id": "uuid", "status": "pending|approved|rejected|expired", "task_description": "string", "risk_level": "string", "approver_id": "string", "created_at": "ISO8601"}`

**Notification System:**  
Approval requests trigger `approval_required` events → Linear workspace hub (PR-68) receives formatted comments with @mentions → Operators notified via Linear (email/mobile/desktop) in <1 second. See `support/docs/NOTIFICATION_SYSTEM.md` for architecture.
