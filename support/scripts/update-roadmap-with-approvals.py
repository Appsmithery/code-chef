...
ORCHESTRATOR_URL = os.getenv("ORCHESTRATOR_URL", "http://45.55.173.72:8001")
    ...
    if phase2_issue:
        ...
        if state != "Done":
            updates_needed.append({
                ...
                "evidence": COMPLETED_WORK["Phase 2: HITL Integration"]["highlights"]
            })
    ...
    if phase5_issue:
        ...
        if state not in ["Done", "In Progress"] or needs_update:
            updates_needed.append({
                ...
                "current_state": state,
                "proposed_state": "In Progress" if "5.2" in phase5_issue["title"] else state,
                "reason": "Phase 5.2 notification system completed (sub-second latency achieved)",
                "evidence": COMPLETED_WORK["Phase 5.2: Notification System"]["highlights"]
            })
...
async def submit_roadmap_update_for_approval(analysis: Dict[str, Any]) -> Dict[str, Any]:
...
async def main():
    ...
    print("   This will trigger an approval notification to @lead-minion in Linear PR-68")
