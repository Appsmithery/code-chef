...
try {
    ...
    # Accept 200 (direct) or 308 (redirect to HTTPS) as success
    if ($statusCode -eq "200" -or $statusCode -eq "308") {
        Write-TestResult -TestName "Frontend serving" -Passed $true -Message "HTTP $statusCode $(if ($statusCode -eq '308') { '(HTTPS redirect)' } else { '' })"
    }
    else {
        Write-TestResult -TestName "Frontend serving" -Passed $false -Message "HTTP $statusCode (expected 200 or 308)"
    }
}
catch {
...
try {
    ...
    foreach ($container in $containers) {
        # Containers are healthy if:
        # 1. Health field is "healthy"
        # 2. Health field is empty (no healthcheck defined) and Status contains "Up"
        $hasHealthcheck = $container.Health -ne ""
        
        if ($hasHealthcheck) {
            $isHealthy = $container.Health -eq "healthy"
        }
        else {
            $isHealthy = $container.Status -match "Up"
        }
        ...
        if ($isHealthy) {
            ...
            Write-TestResult -TestName "Container: $($container.Service)" -Passed $false -Message "$($container.Status) [Health: $($container.Health)]"
        }
    }
}
catch {
...
if ($results.failed -gt 0) {
    Write-Host "[X] Some tests failed. Review issues above." -ForegroundColor Red
    exit 1
}
else {
