...
def run_test_scenario(scenario_name: str, message: str):
    ...
    print(
        f"  Tokens Used: {result['total_tokens']} (vs {result['baseline_tokens']} baseline)"
    )
...
for i, test in enumerate(test_cases, 1):
    print(f"\n[{i}/{len(test_cases)}]")
    result = run_test_scenario(scenario_name=test["name"], message=test["message"])
...
print(
    f"  2. Metadata includes: environment, experiment_group, extension_version, model_version"
)
