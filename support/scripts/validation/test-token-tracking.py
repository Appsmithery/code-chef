...
async def test_token_tracker():
    ...
    assert (
        summary["per_agent"]["orchestrator"]["total_tokens"] == 600
    ), "Orchestrator token count mismatch"
    assert (
        summary["per_agent"]["feature-dev"]["total_tokens"] == 500
    ), "Feature-dev token count mismatch"
...
def test_cost_calculation():
    ...
    ), f"Cost calculation error: expected ${expected_cost}, got ${cost}"
...
def test_config_consistency():
    ...
    for agent_name, config in all_agents.items():
        ...
        assert (
            config.cost_per_1m_tokens > 0
        ), f"{agent_name}: cost_per_1m_tokens must be > 0"
        assert (
            config.max_tokens >= 256
        ), f"{agent_name}: max_tokens must be >= 256 for Gradient"
