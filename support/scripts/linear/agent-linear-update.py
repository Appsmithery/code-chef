...
# Agent identification for workspace traceability and GitHub commit attribution
AGENT_SIGNATURES = {
    "orchestrator": {
        "tag": "@orchestrator-agent",
        "name": "🎯 Orchestrator",
        "signature": "[Orchestrator Agent]",
        "github_identifier": "code-chef",  # Used in commits and PRs
        "commit_author": "code-chef <code-chef@appsmithery.co>",
    },
    "feature-dev": {
        "tag": "@feature-dev-agent",
        "name": "🚀 Feature Dev",
        "signature": "[Feature-Dev Agent]",
        "github_identifier": "code-chef/feature-dev",
        "commit_author": "code-chef feature-dev <feature-dev@code-chef.appsmithery.co>",
    },
    "code-review": {
        "tag": "@code-review-agent",
        "name": "🔍 Code Review",
        "signature": "[Code-Review Agent]",
        "github_identifier": "code-chef/code-review",
        "commit_author": "code-chef code-review <code-review@code-chef.appsmithery.co>",
    },
    "infrastructure": {
        "tag": "@infrastructure-agent",
        "name": "🏗️ Infrastructure",
        "signature": "[Infrastructure Agent]",
        "github_identifier": "code-chef/infrastructure",
        "commit_author": "code-chef infrastructure <infrastructure@code-chef.appsmithery.co>",
    },
    "cicd": {
        "tag": "@cicd-agent",
        "name": "⚙️ CI/CD",
        "signature": "[CI/CD Agent]",
        "github_identifier": "code-chef/cicd",
        "commit_author": "code-chef cicd <cicd@code-chef.appsmithery.co>",
    },
    "documentation": {
        "tag": "@documentation-agent",
        "name": "📚 Documentation",
        "signature": "[Documentation Agent]",
        "github_identifier": "code-chef/documentation",
        "commit_author": "code-chef documentation <documentation@code-chef.appsmithery.co>",
    },
}
...
def update_issue_status(issue_identifier: str, status: str) -> bool:
    ...
    response = requests.post(
        GRAPHQL_ENDPOINT,
        headers={"Authorization": LINEAR_API_KEY, "Content-Type": "application/json"},
        json={"query": query, "variables": {"issueId": issue_identifier}},
    )
    ...
    if result.get("success"):
        issue = result.get("issue", {})
        print(
            f"✅ Updated {issue.get('identifier')} to '{issue.get('state', {}).get('name')}'"
        )
        return True
...
def main():
    ...
    parser_states.add_argument(
        "--team-id", help="Linear team ID (default: Project Roadmaps)"
    )
    ...
    parser_create.add_argument(
        "--project-id", help="Linear project UUID (default: AI DevOps Agent Platform)"
    )
    parser_create.add_argument(
        "--team-id", help="Linear team ID (default: Project Roadmaps)"
    )
    ...
    parser_create.add_argument(
        "--priority", type=int, default=2, choices=[0, 1, 2, 3, 4]
    )
    parser_create.add_argument("--parent-id", help="Parent issue ID for sub-issues")
    parser_create.add_argument(
        "--agent-name",
        help="Agent identifier for traceability (orchestrator, feature-dev, etc.)",
    )

    # Create phase command
    parser_phase = subparsers.add_parser(
        "create-phase", help="Create a phase with sub-tasks"
    )
    parser_phase.add_argument(
        "--phase-number", type=int, required=True, help="Phase number (e.g., 7)"
    )
    ...
    parser_phase.add_argument(
        "--subtasks", required=True, help="Comma-separated subtask titles"
    )
    parser_phase.add_argument(
        "--project-id", help="Linear project UUID (default: AI DevOps Agent Platform)"
    )
    ...
    parser_update.add_argument(
        "--issue-id", required=True, help="Issue identifier (e.g., PR-85)"
    )
    parser_update.add_argument(
        "--status",
        required=True,
    ...
    if args.command == "get-states":
        ...
        team_id = args.team_id if args.team_id else DEFAULT_TEAM_ID
        agent_name = (
            args.agent_name if hasattr(args, "agent_name") and args.agent_name else None
        )
        create_issue(
            title=args.title,
        ...
        update_issue_status(issue_identifier=args.issue_id, status=args.status)
