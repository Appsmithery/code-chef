...
def extract_code_entity(node: ast.AST, file_path: Path, full_content: str, imports: List[str]) -> Dict[str, Any]:
    ...
    if imports:
        content_parts.append(f"Dependencies: {', '.join(imports[:10])}")  # First 10 imports
    
    # Add abbreviated source code (first 500 chars)
    if source_code:
        abbreviated_source = source_code[:500] + ("..." if len(source_code) > 500 else "")
...
async def main():
    ...
    print(f"     -d '{{\"query\": \"workflow execution patterns\", \"collection\": \"{COLLECTION_NAME}\"}}'")
