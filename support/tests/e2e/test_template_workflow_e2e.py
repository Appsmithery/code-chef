...
class TestTemplateWorkflowE2E:
    ...
    async def test_workflow_router_matches_pr_deployment(self, sample_pr_context):
        ...
        from agent_orchestrator.workflows.workflow_router import (
            WorkflowRouter,
            get_workflow_router,
        )
        ...
        selection = await router.select_workflow(task_description)

        assert selection.workflow_name == "pr-deployment"
    ...
    async def test_workflow_router_matches_feature_development(self):
        ...
        assert selection.workflow_name == "feature"
        assert selection.confidence >= 0.7
    ...
    async def test_workflow_router_matches_hotfix(self):
        ...
        assert selection.workflow_name == "hotfix"
        assert selection.confidence >= 0.7
    ...
    async def test_workflow_engine_loads_pr_deployment_template(self):
        ...
        engine = WorkflowEngine(templates_dir="agent_orchestrator/workflows/templates")
    ...
    async def test_graph_routes_to_workflow_executor(self):
        ...
        from graph import WorkflowState, route_entry_point
    ...
    async def test_full_pr_deployment_workflow(
        self, mock_event_bus, mock_llm_client, sample_pr_context
    ):
        ...
        with patch("workflows.workflow_router.get_workflow_router") as mock_router:
            ...
            mock_router.return_value.route = AsyncMock(
                return_value=MagicMock(
                    workflow_id="pr-deployment", confidence=0.95, method="HEURISTIC"
                )
            )
