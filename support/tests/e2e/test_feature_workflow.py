...
""",
    ...
    def mock_mcp_client(self):
        ...
        mock.call_tool = AsyncMock(side_effect=lambda server, tool, params: {
            ("rust-mcp-filesystem", "read_file"): {"content": "# Existing auth code\n\n"},
            ("rust-mcp-filesystem", "write_file"): {"success": True, "path": params.get("path")},
            ("rust-mcp-filesystem", "create_directory"): {"success": True},
            ("gitmcp", "create_branch"): {"branch": "feature/jwt-auth", "success": True},
    ...
    async def test_feature_workflow_high_risk_with_approval(
        ...
        with patch('lib.gradient_client.get_gradient_client', return_value=mock_gradient_client), \
            ...
            initial_state: WorkflowState = {
                "messages": [
                    HumanMessage(content="Add JWT authentication middleware to API endpoints")
                ],
                "current_agent": "",
                "next_agent": "",
                "task_result": {},
            ...
            assert any("feature" in str(m).lower() for m in messages), \
            ...
            if approvals:
                ...
                assert approval.get("approval_issue") == "DEV-135", \
                    "Linear sub-issue should be created"
                assert approval.get("status") == "pending", \
                    "Approval status should be pending"
                assert "🟠" in approval.get("risk_level", "") or "HIGH" in approval.get("risk_level", ""), \
            ...
            assert any("rust-mcp-filesystem" in str(call) for call in tool_calls), \
                "Should use filesystem tools"
            
            # Should have called git tools
            assert any("gitmcp" in str(call) for call in tool_calls), \
