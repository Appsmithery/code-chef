...
"""
    ...
    def mock_mcp_client(self, mock_pr_diff):
        ...
        mock.call_tool = AsyncMock(
            side_effect=lambda server, tool, params: {
                ...
                ("gitmcp", "add_comment"): {"success": True, "comment_id": 12345},
    ...
    async def test_code_review_workflow_no_approval(
        self, mock_mcp_client, mock_gradient_client
    ):
        """
        Test complete code review workflow without HITL approval.
        ...
        with patch(
            ...
            initial_state: WorkflowState = {
                "messages": [
                    HumanMessage(
                        content="Review PR-85 for code quality and security issues"
                    )
                ],
                "current_agent": "",
                "next_agent": "",
            ...
            assert any(
                "review" in str(m).lower() for m in messages
            ...
            assert (
                len(approvals) == 0
            ), "Should not create approval requests for low-risk tasks"
            ...
            assert any(
                "get_pull_request" in call or "get_diff" in call for call in calls
            ), "Should fetch PR data"

            # Check task result
            task_result = result.get("task_result", {})
            if task_result:
                assert "issues_found" in str(task_result) or "review" in str(
                    task_result
    ...
    async def test_pr_diff_analysis(self, mock_mcp_client, mock_gradient_client):
        ...
        with patch("lib.mcp_client.MCPClient", return_value=mock_mcp_client), patch(
            "lib.llm_client.get_llm_client", return_value=mock_gradient_client
        ):

            # Fetch PR diff
            diff_result = await mock_mcp_client.call_tool(
                "gitmcp", "get_diff", {"pr_number": 85}
            )

            assert diff_result["diff"] is not None, "Should return diff content"
            assert (
                "validate_token" in diff_result["diff"]
            ), "Diff should contain code changes"
            ...
            assert any(
                issue["category"] == "security" for issue in analysis["issues"]
    ...
    async def test_review_workflow_execution_time(
        ...
        with patch(
            ...
            assert (
                elapsed < 30
            ), f"Review workflow should complete in < 30s (took {elapsed:.2f}s)"
    ...
    async def test_pr_not_found(self, mock_mcp_client_error):
        ...
        with patch("lib.mcp_client.MCPClient", return_value=mock_mcp_client_error):
            ...
            try:
                ...
                ), "Error should indicate PR not found"
