...
# Use pytest-asyncio instead of pytest-anyio
...
class TestHITLManager:
    ...
    async def test_auto_approve_low_risk(self, manager):
        ...
        assert (
            request_id is None
        ), "Low risk task should auto-approve (no request created)"
    ...
    async def test_authorization_check(self, manager):
        ...
        success = await manager.approve_request(
            request_id=request_id, approver_id="junior.dev", approver_role="developer"
        )
...
class TestWorkflowIntegration:
    ...
    async def test_interrupt_before_approval_gate(self):
        ...
        result = await workflow.ainvoke(
            initial_state, config={"configurable": {"thread_id": "thread-test-1"}}
        )
    ...
    async def test_auto_approve_low_risk_workflow(self):
        ...
        result = await workflow.ainvoke(
            initial_state, config={"configurable": {"thread_id": "thread-test-2"}}
        )
