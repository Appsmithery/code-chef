...
class StoreInsightRequest(BaseModel):
    ...
    source_workflow_id: Optional[str] = Field(None, description="Workflow that generated this insight")
    source_task: Optional[str] = Field(None, description="Task description that led to this insight")
...
class QueryInsightsRequest(BaseModel):
    ...
    agent_id: Optional[str] = Field(None, description="Filter by agent ID (None = cross-agent)")
    ...
    async def prune_expired_insights(self) -> int:
        ...
        try:
            ...
            if to_delete:
                ...
                logger.info(f"Pruned {len(to_delete)} expired insights (TTL: {self.ttl_days} days)")
