...
class WorkflowStateManager:
    ...
    async def connect(self):
        ...
        if not self.pool:
            self.pool = await asyncpg.create_pool(
                self.db_conn_string, min_size=5, max_size=20, command_timeout=60
            )
    ...
    def _get_pool(self) -> asyncpg.Pool:
        """Get database connection pool with type assertion.

        Raises:
            RuntimeError: If pool is not initialized (connect() not called)
        """
    ...
    async def update_state(
        self, workflow_id: str, updates: Dict[str, Any], agent_id: Optional[str] = None
    ):
        """
        ...
        for attempt in range(max_retries):
            async with self._get_pool().acquire() as conn:
                ...
                current_state = (
                    json.loads(current_state_raw)
                    if isinstance(current_state_raw, str)
                    else current_state_raw
                )
                ...
                if result == "UPDATE 1":
                    logger.info(
                        f"Updated workflow {workflow_id} state (by {agent_id or 'system'})"
                    )
                    return

                # If we get here, version mismatch occurred
                logger.warning(
                    f"Concurrent update detected for workflow {workflow_id}, retrying ({attempt + 1}/{max_retries})"
                )
                await asyncio.sleep(0.1 * (attempt + 1))  # Exponential backoff

        raise RuntimeError(
            f"Failed to update workflow {workflow_id} after {max_retries} attempts due to concurrent modifications"
        )

    async def update_step(
        self, workflow_id: str, current_step: str, agent_id: Optional[str] = None
    ):
        """
    ...
    async def restore_checkpoint(
        self, workflow_id: str, checkpoint_id: int
    ) -> Dict[str, Any]:
        """
