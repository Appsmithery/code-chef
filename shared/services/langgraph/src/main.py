...
class WorkflowRequest(BaseModel):
    ...
    task_description: str = Field(
        ..., description="Task description for workflow routing"
    )
    ...
    thread_id: Optional[str] = Field(
        None, description="Thread ID for workflow checkpointing"
    )
    enable_checkpointing: bool = Field(
        True, description="Enable PostgreSQL state persistence"
    )
...
class StreamRequest(BaseModel):
    ...
    request_payloads: Optional[Dict[str, Any]] = Field(
        None, description="Agent-specific request payloads"
    )
    thread_id: Optional[str] = Field(
        None, description="Thread ID for workflow checkpointing"
    )
    ...
    stream_mode: str = Field(
        "values", description="Streaming mode: values, updates, or debug"
    )
...
async def stream_workflow_events_endpoint(request: StreamRequest):
    ...
    async def event_generator() -> AsyncIterator[str]:
        ...
        try:
            logger.info(
                f"Starting detailed event stream: {request.task_description[:100]}"
            )
